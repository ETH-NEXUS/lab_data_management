...
class MappingTest(TestCase):
    ...
    def test_positionMapping(self):
        plateDimension = PlateDimension.objects.create(name='bla', rows=2,
                                                       cols=3)
        ...
        plateDimension = PlateDimension.objects.create(name='bla', rows=4,
                                                       cols=5)
        ...
        plateDimension = PlateDimension.objects.create(name='bla', rows=16,
                                                       cols=22)
...
class PlateTest(TestCase):
    ...
    def setUp(self):
        self.dimension = PlateDimension.objects.create(name='dim_3x2', cols=3,
                                                       rows=2)
        self.sourcePlate = Plate.objects.create(barcode='000001',
                                                dimension=self.dimension, )
        for i in range(self.dimension.num_wells):
            comp = Compound.objects.create(identifier=f"comp{i}",
                                           structure='c=c')
            well = Well.objects.create(plate=self.sourcePlate, position=i)
            WellCompound.objects.create(well=well, compound=comp, amount=1)
        self.targetPlate = Plate.objects.create(barcode='000002',
                                                dimension=self.dimension)
    ...
    def test_plate_mapping(self):
        ...
        self.assertEqual('comp2', targetWells[
            3].donors.first().well.compounds.first().identifier)
    ...
    def test_plate_copy(self):
        ...
        for p in range(len(targetWells)):
            self.assertEqual(f"comp{p}",
                             targetWells[p].compounds.first().identifier)
    ...
    def test_plate_mapping_from_csv(self):
        ...
        with open(csv_file, 'w', newline='') as cf:
            writer = csv.DictWriter(cf, ('from', 'to', 'amount'),
                                    delimiter=delimiter)
            writer.writeheader()
            writer.writerow({'from': 0, 'to': 5, 'amount': 10})
            writer.writerow({'from': 1, 'to': 4, 'amount': 20})

        mappingList = MappingList.from_csv(csv_file, 'from', 'to', 'amount',
                                           delimiter=';')
    ...
    def test_plate_mapping_from_csv_with_illegal_values(self):
        ...
        self.assertRaises(ValueError, MappingList.from_csv, csv_file, 'from',
                          'to', 'amount', delimiter=';')
    ...
    def test_only_library_or_experiment_or_template_constraint(self):
        ...
        try:
            with transaction.atomic():
                Plate.objects.create(barcode='bar1', dimension=self.dimension,
                                     library=library, experiment=experiment,
                                     template=template)
        ...
        try:
            with transaction.atomic():
                Plate.objects.create(barcode='bar2', dimension=self.dimension,
                                     library=library, experiment=experiment)
            self.fail("Should raise an IntegrityError")
        except IntegrityError:
            pass
        try:
            with transaction.atomic():
                Plate.objects.create(barcode='bar3', dimension=self.dimension,
                                     experiment=experiment, template=template)
            self.fail("Should raise an IntegrityError")
        except IntegrityError:
            pass
        try:
            with transaction.atomic():
                Plate.objects.create(barcode='bar4', dimension=self.dimension,
                                     library=library, template=template)
            self.fail("Should raise an IntegrityError")
        except IntegrityError:
            pass
    ...
    def test_create_library_plate(self):
        ...
        library = CompoundLibrary.objects.create(name='CL')
        plate = Plate.objects.create(barcode='123456',
                                     dimension=self.dimension, library=library)
...
class WellTest(TestCase):
    ...
    def setUp(self):
        self.dimension = PlateDimension.objects.create(name='dim_3x2', cols=3,
                                                       rows=2)
        self.plate = Plate.objects.create(barcode='123456789',
                                          dimension=self.dimension, )
    ...
    def test_source_plate_discovery(self):
        ...
        plate1 = Plate.objects.create(barcode='0001',
                                      dimension=self.dimension, )

        plate2 = Plate.objects.create(barcode='0002',
                                      dimension=self.dimension, )

        plate3 = Plate.objects.create(barcode='0003',
                                      dimension=self.dimension, )
        ...
        for well in plate3.wells.all():
            ...
            self.assertEqual(plate1,
                             well.donors.first().well.donors.first().well.plate)
