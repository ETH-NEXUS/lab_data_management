...
from .models import (Well, Plate, Measurement, WellWithdrawal, WellCompound,
                     PlateMapping, Experiment, BarcodeSpecification,
                     PlateDimension, Project)
from .serializers import (PlateSerializer, WellSerializer,
                          PlateMappingSerializer, SimpleExperimentSerializer,
                          ExperimentSerializer, ProjectSerializer)
...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        experiments = Prefetch('experiments',
                               queryset=Experiment.objects.all())
        plates = Prefetch('plates',
                          queryset=Plate.objects.all().order_by('barcode'))
        return Project.objects.all().prefetch_related(
          experiments).prefetch_related(plates)
...
class PlateViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        measurements = Prefetch('measurements',
                                queryset=Measurement.objects.all())
        withdrawals = Prefetch('withdrawals',
                               queryset=WellWithdrawal.objects.select_related(
                                   'target_well').all())
        donors = Prefetch('donors',
                          queryset=WellWithdrawal.objects.select_related(
                              'well').all())
        well_compounds = Prefetch('well_compounds',
                                  queryset=WellCompound.objects.select_related(
                                      'compound').all())
        wells = Prefetch('wells',
                         queryset=Well.objects.select_related('sample',
                                                              'type').order_by(
                             'position').prefetch_related(
                             well_compounds).prefetch_related(
                             withdrawals).prefetch_related(
                             donors).prefetch_related(measurements))
        return Plate.objects.select_related('dimension', 'experiment',
                                            'library').prefetch_related(wells)
    ...
    def barcodes(self, request):
        ...
        return Response([{'label': plate.barcode, 'value': plate.id,
            'library': SimpleCompoundLibrarySerializer(
                plate.library).data if plate.library else None,
            'experiment': SimpleExperimentSerializer(
                plate.experiment).data if plate.experiment else None} for plate
                            in Plate.objects.filter(predicate)])
    ...
    def chain(self, request, pk=None):
        ...
        def appendDonors(nodes, edges, well):
            for donor in well.donors.all():
                nodes.update(
                    {node_key(donor.well): {'name': node_name(donor.well)}})
                edge_key = str(uuid4())
                edges.update({edge_key: {'source': node_key(donor.well),
                    'target': node_key(well), 'label': donor.amount}})
        ...
        def appendWithdrawals(nodes, edges, well):
            for withdrawal in well.withdrawals.all():
                if (withdrawal.target_well):
                    nodes.update({node_key(withdrawal.target_well): {
                        'name': node_name(withdrawal.target_well)}})
                    edge_key = str(uuid4())
                    edges.update({edge_key: {'source': node_key(well),
...
class ExperimentViewSet(viewsets.ModelViewSet):
    ...
    def bulk_add_plates(self, request):
        try:
            ...
            barcode_specification_id = int(
                request.data['barcode_specification_id'])
            ...
            return Response({'error': 'Barcode specification not found'},
                            status=status.HTTP_404_NOT_FOUND)
        except PlateDimension.DoesNotExist:
            return Response({'error': 'Plate dimension not found'},
                            status=status.HTTP_404_NOT_FOUND)
        ...
        for i in range(number_of_plates):
            plate = Plate(
                ...
                experiment=experiment, dimension=plate_dimension)
            plates.append(plate)
        ...
        try:
            ...
            return Response({'error': 'Invalid plate data'},
                            status=status.HTTP_400_BAD_REQUEST)

        return Response({'success': 'Plates added successfully'},
                        status=status.HTTP_200_OK)
