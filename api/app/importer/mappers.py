...
class EchoMapper(BaseMapper):
    ...
    def map(self, data: List[Dict], **kwargs):
        try:
            ...
            raise MappingError(
                f"Source plate with barcode does not "
                f"exist."
                )
        try:
        ...
        if mapping_success:
            ...
            plate_mapping.mapping_file.save(
    ...
    def __create_plate_by_name_and_barcode(self, plate_name: str,
        ...
        return Plate.objects.create(
            barcode=barcode, experiment=barcode_specification.experiment,
...
class MeasurementMapper(BaseMapper):
    ...
    def parse(self, filename, file, **kwargs):
        ...
        metadata_list = self.__parse_measurement_metadata(
            data[metadata_start_index:]
        )

        return {
            "barcode": barcode, "filename": filename, "measurement_data": {
                    ...
                    if well:
                        for index, value in enumerate(values):
                            measurement = Measurement.objects.create(
                                well=well, value=value, identifier=identifier,
