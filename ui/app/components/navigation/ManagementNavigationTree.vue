...
const getSlotItemPath = (item: unknown): string => {
  ...
  // Primary shape used by this component.
  const directItemPath = (item as { itemPath?: unknown }).itemPath
  if (typeof directItemPath === 'string' && directItemPath.trim() !== '') {
    return directItemPath
  }

  // Runtime fallback for tree payloads that expose `path`.
  const directPath = (item as { path?: unknown }).path
  if (typeof directPath === 'string' && directPath.trim() !== '') {
    return directPath
  }

  // Runtime fallback for payloads wrapped under `value`.
  const nestedValue = (item as { value?: unknown }).value
  if (typeof nestedValue === 'object' && nestedValue !== null) {
    const nestedItemPath = (nestedValue as { itemPath?: unknown }).itemPath
    if (typeof nestedItemPath === 'string' && nestedItemPath.trim() !== '') {
      return nestedItemPath
    }

    const nestedPath = (nestedValue as { path?: unknown }).path
...
const handleDragStart = (path: string, event: DragEvent) => {
  ...
  // Add multiple text flavors so desktop browsers consistently keep payload.
      ...
      <template #management-path-node-label="{ item }">
        <UContextMenu :items="getPathContextMenuItemsFromSlotItem(item)">
          <span
            ...
            draggable="true"
            @dragstart.stop="handleDragStart(getSlotItemPath(item), $event)"
          >
