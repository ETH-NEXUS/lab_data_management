...
import { useInventoryCheckInOutHistoryPageQuery } from '~/composables/inventory/useInventoryCheckInOutHistoryPageQuery'
...
const historyQuery = useInventoryCheckInOutHistoryPageQuery(historyPageQueryParams)
...
<template>
  <section class="space-y-3">
    <div class="space-y-1">
      <p class="inventory-section-title">{{ t('inventory.check_in_out_workspace.title') }}</p>
    ...
    <UCard :ui="{ root: 'core-card divide-y divide-slate-200/70' }">
      <p v-if="historyQuery.isPending.value" class="text-sm text-slate-600">
        {{ t('inventory.check_in_out_workspace.loading') }}
      </p>
      <p v-else-if="errorMessage" class="text-sm text-red-600">
        {{ errorMessage }}
      </p>
      <template v-else>
        <InventoryHistoryTable :records="records" mode="check_in_out" @select-record="openRecord" />
